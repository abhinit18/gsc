...
function analyticsController($scope, $http, $location, baseUrl, growl,$rootScope) {
    $scope.activeTab = 0;
    ...
    $http({method: 'GET', url: baseUrl + '/omsservices/webapi/orderreports/orderscsv'})
        .then(function (response) {
            ...
            return $http({method: 'GET', url: baseUrl + '/omsservices/webapi/inventoryreports/inventorycsv'})
        })
        .then(function (response) {
            if (response.status == 200 && response.data.length) {
        ...
        .catch(function (data, status, headers, config) {
            $scope.showOrdersLoader = false;
            $scope.showInventoryLoader = false;
            growl.error("Something went wrong.Please try again after sometime.", {ttl: 3000});
    ...
    $scope.showOrdersChart = function (range, filteredDate, filtererUnitPrice) {
        ...
        if (!( filtererUnitPrice && Object.keys(filtererUnitPrice).length )) {
            if ($scope.perUnitPriceDim.top(Infinity).length) {
                ...
                $scope.perUnitPrice.options = {
                    floor: Number(minUnitPrice),
                    ceil: Number(maxUnitPrice),
                    minRange: Number(minUnitPrice),
        ...
        $scope.channel = $scope.generatePieChart($scope.channel, $scope.channelDim, remove_empty_bins_custom($scope.channelGroup, $scope.allFilters.channelSelected), pieWidth, 'channel');
        $scope.state = $scope.generatePieChart($scope.state, $scope.stateDim, remove_empty_bins_custom($scope.stateGroup, $scope.allFilters.stateSelected), pieWidth, 'state');
        $scope.category = $scope.generatePieChart($scope.category, $scope.categoryDim, remove_empty_bins_custom($scope.categoryGroup, $scope.allFilters.categorySelected), pieWidth, 'category');
        $scope.item = $scope.generatePieChart($scope.item, $scope.itemDim, remove_empty_bins_custom($scope.itemGroup, $scope.allFilters.itemSelected), pieWidth, 'item');
        $scope.customer = $scope.generatePieChart($scope.customer, $scope.customerDim, remove_empty_bins_custom($scope.customerGroup, $scope.allFilters.customerSelected), pieWidth, 'customer');
    ...
    $scope.showInventoryChart = function (itemCost, filteredDate) {
        ...
        if(!( itemCost && Object.keys(itemCost).length )){
            ...
            $scope.totalCountOfAllSku = angular.copy(sumOfOrderValue($scope.totalSkuGroup.value().skuCodes));
        ...
        //Generating All the charts and Number Displays
        $scope.warehouse = $scope.generatePieChart($scope.warehouse, $scope.warehouseDim, remove_empty_bins_custom($scope.warehouseGroup, $scope.allFilters.warehouseSelected), pieWidth, 'warehouse');
        $scope.sku = $scope.generatePieChart($scope.sku, $scope.skuDim, remove_empty_bins_custom($scope.skuGroup, $scope.allFilters.skuSelected), pieWidth, 'sku');
        $scope.categoryName = $scope.generatePieChart($scope.categoryName, $scope.categoryNameDim, remove_empty_bins_custom($scope.categoryNameGroup, $scope.allFilters.categoryNameSelected), pieWidth, 'categoryName');
    ...
    $scope.generatePieChart = function (pieChart, dimension, group, chartWidth, chartFor) {
        ...
        if (chartFor == 'channel' || chartFor == 'state' || chartFor == 'category' || chartFor == 'item' || chartFor == 'customer') {
            ...
            labelAccessor = function (d) {
                if(d.key !== "empty"){
                    ...
                    return "Rs." + ((Math.round(d.value.orderValue ? d.value.orderValue : d.value) <= 0) ? 0 : Math.round(d.value.orderValue ? d.value.orderValue : d.value));
                }
                else{
                    return d.key ;
                }
        ...
        pieChart
            .width(chartWidth - 10)
            .height(chartWidth - (chartWidth / 3))
            .radius((((chartWidth) / 2.1) - 60 ))
            .innerRadius(((chartWidth) / 3.2) - 60 )
                ...
                if (!(chart.select('g #extra-label')[0][0])) {
                    ...
                    text.enter().append('text')
                        .attr({
                            ...
                            'y': extra_data[1].y < 200 ? extra_data[1].y : extra_data[1].y + 10
                        })
            ...
            .valueAccessor(
                function (d) {
                    switch (id) {
                        ...
                        case '#totalSku' :
                            var returnValue = sumOfOrderValue(d.skuCodes);
                            //var returnValue = d;
                            break;
                        case '#totalSkuCost':
                        ...
                        case '#totalSkuPercentage':
                            var returnValue = sumOfOrderValue(d.skuCodes) / total;
                            break;
