...
shippingpartnersController.$inject = ['$scope', '$http', '$location', 'baseUrl', 'growl','$cookies','downloadAWBTemplateUrl','downloadRatesTemplateUrl','$timeout','Upload'];

function shippingpartnersController($scope, $http, $location, baseUrl, growl,$cookies,downloadAWBTemplateUrl,downloadRatesTemplateUrl,$timeout,Upload) {
    $scope.addNewShippingPartnerClicked = false;
    $scope.isShippingName = false;
    $scope.isMetaEntered = [];
    $scope.shipPartnerMode = 'add';
    $scope.downloadAWBTemplateUrl = downloadAWBTemplateUrl;
    $scope.downloadRatesTemplateUrl = downloadRatesTemplateUrl;
    ...
    $scope.saveSubShippingPartner = function(subpartnerData, partnerData, metapartnerData) {
        ...
        if (subpartnerData.tableShippingCarrierConfigName != null) {
            ...
            if(subpartnerData.tableShippingCarrierConfigName.length > 45) {
                growl.error("Shipping Partner Name is exceeding the 45 characters limit!");
                $scope.isShippingName = true;            
            }
        }
    ...
    $scope.editSubShippingPartner = function(subpartnerData, partnerData, metapartnerData) {
        ...
        if ($scope.metaSubmitEnable == false && $scope.isShippingName == false) {
            ...
            $http({
                method: 'PUT',
                url: baseUrl + '/omsservices/webapi/shippingcarrierconfigs/' + subpartnerData.idtableClientShippingCarrierConfigId,
                data: postSubpartnerData,
                headers: {
                    'Content-Type': 'application/json'
                }
                ...
                if(status == 400){
                    ...
                    growl.error("Shipping Partner can't Be Updated!");
                }
            });
        }
    }
    ...
    $scope.uploadBulkAwbOrderFile = function(configId,bulkOrderUploadfile) {
        ...
        if (file) {
            if (!file.$error) {
                var uploadUrl = baseUrl + '/omsservices/webapi/shippingcarrierconfigs/'+configId+'/uploadawb';
                ...
                upload.then(function(resp) {
                    ...
                    growl.success("File has been uploaded successfully.");
                }, function(resp) {
                    console.log(resp);
