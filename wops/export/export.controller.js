/**
 * Created by angularpc on 15-02-2017.
 */
myApp.controller("exportController", exportController);

exportController.$inject = ['$rootScope' , 'PagerService',"$scope", "$http", "$location", "baseUrl", "$mdDialog", "$mdMedia", "growl", "$window", "Upload", "PagerService", "$q",  "$routeParams", "$cookies", "downloadOrderTemplateUrl","downloadSkuTemplateUrl","downloadMastersTemplateUrl","downloadCustomersTemplateUrl","downloadVendorsTemplateUrl","downloadPOTemplateUrl", "downloadBulkCancelTemplateUrl" , "$timeout",'commonPathUrl','$cookies'];

function exportController($rootScope,PagerService,$scope, $http, $location, baseUrl, $mdDialog, $mdMedia, growl, $window, Upload, PagerService, $q, $routeParams, $cookies,downloadOrderTemplateUrl,downloadSkuTemplateUrl,downloadMastersTemplateUrl,downloadCustomersTemplateUrl,downloadVendorsTemplateUrl,downloadPOTemplateUrl,downloadBulkCancelTemplateUrl,$timeout,commonPathUrl,$cookies) {

    $scope.searchSuccessClicked = false;

    $scope.filePathUrl = commonPathUrl;
    $scope.selectedTab = 0;
    ...
    $scope.loadExportCounting = function(page,state) {
        var successCountUrl = baseUrl + '/omsservices/webapi/export/filtercount?status=success&entity='+state;
        var inprogressCountUrl = baseUrl + '/omsservices/webapi/export/filtercount?status=inprogress&entity='+state;
    ...
    $scope.loadExportSuccessData = function(exportState){
        var successUrl = baseUrl + '/omsservices/webapi/export?start=0&size=5&status=success&entity='+exportState;
    ...
    $scope.loadExportProgressData = function(exportState){
        var inprogressUrl = baseUrl + '/omsservices/webapi/export?start=0&size=5&status=inprogress&entity='+exportState;
